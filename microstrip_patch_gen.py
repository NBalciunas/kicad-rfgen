...
def generate_microstrip_patch(name="PATCH_ANTENNA", patch_length=15, patch_width=20, feed_length=5, feed_width=3, inset_gap=1, inset_distance_input=5, ground_check=True, mask_check=True):
    points_f = [
        ...
        pcbnew.VECTOR2I(pcbnew.FromMM(feed_width/2 + inset_gap), pcbnew.FromMM(patch_length/2 - inset_distance_input)),
    ...
    pad_str = f"(pad 1 smd rect (at 0 {patch_length/2+feed_length-feed_width/2}) " \
              f"(size {feed_width} {feed_width}) (layers F.Cu))\n"
