    ...
    def update_fields(self, choice):
        ...
        match choice:
            case "Microstrip Patch Antenna (Inset Feed)":
                self.img = add_image(self.panel, self.dynamic_box, "assets/microstrip-patch-inset-antenna.png", size=(270, 270))
            ...
            case "Microstrip Patch Antenna (Coaxial Feed)":
                ...
                self.footprint_name_input = add_field(self.panel, self.dynamic_box, "footprint name", "PATCH_ANTENNA", width=160)
                self.patch_length_input = add_field(self.panel, self.dynamic_box, "Patch Length (Lp)", "28.348", width=80, unit="mm")
                self.patch_width_input = add_field(self.panel, self.dynamic_box, "Patch Width (Wp)", "28.348", width=80, unit="mm")
                self.ground_length_input = add_field(self.panel, self.dynamic_box, "Ground Length (Lg)", "37.948", width=80, unit="mm")
                self.ground_width_input = add_field(self.panel, self.dynamic_box, "Ground Width (Wg)", "37.948", width=80, unit="mm")
                self.feed_offset_x_input = add_field(self.panel, self.dynamic_box, "Feed Offset X (Xf)", "0", width=80, unit="mm")
                self.feed_offset_y_input = add_field(self.panel, self.dynamic_box, "Feed Offset Y (Yf)", "7.286", width=80, unit="mm")
                self.pad_radius_input = add_field(self.panel, self.dynamic_box, "Pad Radius (Rp)", "2.143", width=80, unit="mm")
                self.hole_radius_input = add_field(self.panel, self.dynamic_box, "Hole Radius (Rh)", "0.635", width=80, unit="mm")
                self.clearance_radius_input = add_field(self.panel, self.dynamic_box, "Clearance Radius (Rc)", "3.5", width=80, unit="mm")
                self.ground_check_input = add_checkbox(self.panel, self.dynamic_box, "Include ground plane", default=True)
                self.mask_check_input = add_checkbox(self.panel, self.dynamic_box, "Remove solder mask", default=True)
            case "Microstrip Patch Antenna (Dual Coaxial Feed)":
                ...
                self.ground_length_input = add_field(self.panel, self.dynamic_box, "Ground Length (Lg)", "37.914", width=80, unit="mm")
                self.ground_width_input = add_field(self.panel, self.dynamic_box, "Ground Width (Wg)", "37.914", width=80, unit="mm")
                self.feed_offset_x_a_input = add_field(self.panel, self.dynamic_box, "Feed Offset X A (Xfa)", "7.286", width=80, unit="mm")
                self.feed_offset_y_a_input = add_field(self.panel, self.dynamic_box, "Feed Offset Y A (Yfa)", "0", width=80, unit="mm")
                self.feed_offset_x_b_input = add_field(self.panel, self.dynamic_box, "Feed Offset X B (Xfb)", "0", width=80, unit="mm")
                self.feed_offset_y_b_input = add_field(self.panel, self.dynamic_box, "Feed Offset Y B (Yfb)", "7.286", width=80, unit="mm")
                self.pad_radius_input = add_field(self.panel, self.dynamic_box, "Pad Radius (Rp)", "2.143", width=80, unit="mm")
                self.hole_radius_input = add_field(self.panel, self.dynamic_box, "Hole Radius (Rh)", "0.635", width=80, unit="mm")
                self.clearance_radius_input = add_field(self.panel, self.dynamic_box, "Clearance Radius (Rc)", "3.5", width=80, unit="mm")
                self.ground_check_input = add_checkbox(self.panel, self.dynamic_box, "Include ground plane", default=True)
            ...
            case "Microstrip Patch Antenna (Legacy)":
                self.img = add_image(self.panel, self.dynamic_box, "assets/microstrip-patch-legacy-antenna.png", size=(270, 270))
                self.footprint_name_input = add_field(self.panel, self.dynamic_box, "footprint name", "PATCH_ANTENNA", width=160)
                self.patch_length_input = add_field(self.panel, self.dynamic_box, "Patch Length (Lp)", "11.6", width=80, unit="mm")
                self.patch_width_input = add_field(self.panel, self.dynamic_box, "Patch Width (Wp)", "16.3", width=80, unit="mm")
